...
// rmi = rotationMatrixIndex
inline unsigned long rmi( unsigned long i, unsigned long j) {
  return i*3 + j;
}
...
inline geometry_msgs::Quaternion toQuaternion( const double* m ) {
  tf2::Matrix3x3 tfM(m[0],m[1],m[2],m[3],m[4],m[5],m[6],m[7],m[8]);
  /* ROS_INFO_STREAM(m[0]<<" "<<m[1]<<" "<<m[2]<<" "); */
  tf2::Quaternion tfQ;
  tfM.getRotation(tfQ);

  tf2::Quaternion rotate;
  rotate.setRPY(0,M_PI/2,0);

  geometry_msgs::Quaternion q = tf2::toMsg(tfQ*rotate); 
  /* Q.w = sqrt( max( 0, 1 + m[rmi(0,0)] + m[rmi(1,1)] + m[rmi(2,2)]) ) / 2; */
  /* Q.x = sqrt( max( 0, 1 + m[rmi(0,0)] - m[rmi(1,1)] - m[rmi(2,2)] ) ) / 2; */
  /* Q.y = sqrt( max( 0, 1 - m[rmi(0,0)] + m[rmi(1,1)] - m[rmi(2,2)] ) ) / 2; */
  /* Q.z = sqrt( max( 0, 1 - m[rmi(0,0)] - m[rmi(1,1)] + m[rmi(2,2)] ) ) / 2; */
  /* Q.x = copysign( Q.x, m[rmi(2,1)] - m[rmi(1,2)] ); */
  /* Q.y = copysign( Q.y, m[rmi(0,2)] - m[rmi(2,0)] ); */
  /* Q.z = copysign( Q.z, m[rmi(1,0)] - m[rmi(0,1)] ); */ 
  return q;
}
...
void Ros::run(int argc, char **argv) {
  ...
  Node* robotNode = supervisor->getFromDef(mRobotName);
  posePublisher = mNodeHandle->advertise<geometry_msgs::PoseStamped>(mRobotName + "/pose",1);
  uint32_t seq = 0;
  ...
  while (!mEnd && ros::ok()) {
    ...
    // publish pose
    const double* position = robotNode->getPosition();
    geometry_msgs::Point rosPosition;
    rosPosition.x = position[0];
    rosPosition.y = position[1];
    rosPosition.z = position[2];

    const double* orientation = robotNode->getOrientation();
    geometry_msgs::Quaternion rosOrientation = toQuaternion(orientation);

    geometry_msgs::PoseStamped pose;
    pose.pose.position = rosPosition;
    pose.pose.orientation = rosOrientation;
    pose.header.seq = seq++;
    ...
    transform.setOrigin( tf::Vector3(rosPosition.x, rosPosition.y, rosPosition.z) );
    tf::Quaternion q(rosOrientation.x,rosOrientation.y,rosOrientation.z,rosOrientation.w);
    transform.setRotation(q);
    tBcast.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "map", mRobotName));

    transform.setIdentity();
    transform.setRotation(tf::Quaternion(tf::Vector3(1, 0, 0), M_PI/2));
    tBcast.sendTransform(tf::StampedTransform(transform, ros::Time::now(), mRobotName, mRobotName + "/laser"));
